package com.hexing.bluetooth.protocol.dlms;
...
public class HXHdlcDLMS implements IProtocol {
     ...
     * @return
     */
    private boolean LinkNoAuth(HXFramePara fpara, ICommAction commDevice) throws IOException {
        byte[] sndByt = null;
        byte[] receiveByt = null;
        boolean isSend = false;
        Nrec = 0;
     ...
     * @return
     */
    private boolean LinkLLSAuth(HXFramePara fpara, ICommAction commDevice) throws IOException {
        byte[] sndByt = null;
        byte[] receiveByt = null;
        boolean isSend = false;
        Nrec = 0;
     ...
     * @return
     */
    private boolean LinkHLSAuth(HXFramePara fpara, ICommAction commDevice) throws IOException {
        byte[] sndByt = null;
        byte[] receiveByt = null;
        boolean isSend = false;
        Nrec = 0;
    ...
    @Override
    public byte[] Read(HXFramePara paraModel, ICommAction commDevice) throws IOException {
//		//光电只握手一次，帧序号，不可清零
        ...
        paraModel.Nrec = Nrec;
        paraModel.Nsend = Nsend;
        paraModel.decDataType = dlmsService.DataType.Struct_Billing;//临时测试
        sndByt = hdlcframe.getReadRequestNormalFrame(paraModel);
        Log.d("send->", Arrays.toString(sndByt));
        Nsend++;
        paraModel.Nsend = Nsend;
        receiveByt = commDevice.ReceiveByt(paraModel.SleepT,
                paraModel.WaitT);
        Log.d("send->", Arrays.toString(receiveByt));
    ...
    @Override
    public byte[] convertSendLowBluetooth(HXFramePara paraModel, ICommAction commDevice) throws IOException {
        commParaModel = paraModel;
        byte[] sndByt;
        frameCnt++;
        paraModel.frameCnt = frameCnt;
    ...
    @Override
    public byte[] convertReceiverLowBluetooth(HXFramePara paraModel, ICommAction commDevice) throws IOException {
        ...
        byte[] receiveByt = commDevice.ReceiveByt(paraModel.SleepT,
                paraModel.WaitT);
        Log.d("receive->", Arrays.toString(receiveByt));
        if (paraModel.enLevel != 0x00)// 如果有加密或认证，则将收到数据还原
        {
            receiveByt = hdlcframe.GetOriginalData(receiveByt, paraModel);
        }
    ...
    @Override
    public boolean Write(HXFramePara paraModel, ICommAction commDevice) throws IOException {
        commParaModel = paraModel;//
        paraModel.Nsend = Nsend;
        paraModel.Nrec = Nrec;
        byte[] sndByt = null;
    ...
    @Override
    public boolean Action(HXFramePara paraModel, ICommAction commDevice) throws IOException {
        commParaModel = paraModel;//
        paraModel.Nsend = Nsend;
        paraModel.Nrec = Nrec;
